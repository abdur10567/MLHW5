...
for index, row in rules.iterrows():
    antRule = ""
    for antecedent in list(row["antecedents"]):
        antRule += antecedent + ", "
    antRule = antRule[:-2]
    print(antRule, end=" ")
    conRule = "-> "
    for consequent in list(row["consequents"]):
        conRule += consequent + ", "
    conRule = conRule[:-2]
    print(conRule)
    print("Support:", row["support"])
    print("Confidence:", row["confidence"])
    print("Prior:", row["consequent support"])
    print("Gain in Confidence: " + str(100 * (row["confidence"] - row["consequent support"]) / row["consequent support"]))
    print("\n")
